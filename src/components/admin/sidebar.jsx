...
const Sidebar = () => {
  const menuList = [
    ...
    { name: "Inquiry List", link: "/admin/inquiry" },
    { name: "Contact List", link: "/admin/contact" },
  ];
        ...
        <ul className="flex flex-col gap-3">
          {menuList?.map((ele, ind) => (
            <li
              ...
              className={
                ele?.link === pathname
                  ? "shadow-[inset_4px_0px_0px_0px_#fff]"
                  : ""
              }
            >
              <Link
                to={ele?.link ? ele?.link : "#"}
                className={`pl-4 w-full block ${
                  ele?.link === pathname ? "font-bold" : ""
                }`}
              >
                {ele?.name}
              </Link>
