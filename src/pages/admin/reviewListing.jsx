...
import { addReview, deleteReview, getReviewList } from '../../redux/actions/reviewAction';
...
const ReviewListing = () => {
  ...
  const columns = useMemo(
    () => [
      ...
      },  
      {
        ...
        Cell: ({ row}) => (
          <div className='w-fit'>
          <Button name='Delete' btnClass="secondaey" onClick={()=>deleteReviewHandler(row?.original?._id)}/>
          </div>
        ),
      },
    ],
  ...
  const {loading,reviewList,message,error}=useSelector(state=>state.review)
  ...
  useEffect(()=>{dispatch(getReviewList())},[dispatch,message])
  ...
  return (
    <AdminMain pageName={'Reviews'}>
      ...
      {modal && (
        <Modal setModal={modalHandler} heading={'Add New Review'} maxWidth={'max-w-[50%]'}>
          <div className="p-4">
